...
Mouse::Mouse(HWND &hwnd)	{
	...
	ZeroMemory( lastClicktimePeriod, sizeof(float) * MOUSE_MAX );
		...
		*/
		...
		for(int i = 0; i < 5; ++i) {
			if(raw->data.mouse.usButtonFlags & buttondowns[i])	{
				...
				*/
				lastClicktimePeriod[i] = clickLimit;
			}
...
/*
Updates the double click timePeriodrs for each mouse button. msec is milliseconds
since the last UpdateDoubleClick call. timePeriodrs going over the double click
limit set the relevant double click value to false.
*/
void Mouse::UpdateDoubleClick(float msec)	{
	for(int i = 0; i < MOUSE_MAX; ++i) {
		if(lastClicktimePeriod[i] > 0) {
			lastClicktimePeriod[i] -= msec;
			if(lastClicktimePeriod[i] <= 0.0f) {
				doubleClicks[i]  = false;
				lastClicktimePeriod[i] = 0.0f;
			}
		}
	}	
}